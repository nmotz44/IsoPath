---
title: "RpathSIA Workflow Walkthrough"
...
if (!require(devtools)) install.packages('devtools')
library(devtools)
if (!require(RpathSIA2)) install_github('nmotz44/IsoPath') # uncomment when this is on github
```

## Setup Workspace

Set up the workspace to run the package and models. This includes clearing any previously saved variables and loading the required packages. The packages needed are: *simmr*, *Rpath*, and *RpathSIA2*. *dplyr* and *vioplot* are needed for functions within *RpathSIA2* but will be automatically loaded with the package, please make sure these packages are installed. In order to load and use *simmr*, *JAGS* must be installed to your operating system. Please see information from the simmr package (https://cran.r-project.org/web/packages/simmr/vignettes/simmr.html), and JAGS (https://sourceforge.net/projects/mcmc-jags/). Lastly, we will load the isotope data (iso_data) that is needed to run the example model used in the paper. This data is within the *RpathSIA2* package and will be loaded below using the function "example_data".
...
This section will pull out the average diet from the *simmr* outputs to pass these to *Rpath* for the intial model run. First, a list called "simmr_list" is made of all the *simmr* outputs generated in the initial for loop. Then, this for loop will use the simmr_diet_output_mean function from *RpathSIA2* to make the diet information from *simmr* in the correct format to pass to *Rpath*. The final step of the loop will save this output to be put into *Rpath* later. 
